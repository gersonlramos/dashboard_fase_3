...
<div style="text-align: center; background: linear-gradient(90deg, #1f77b4 0%, #2ca02c 100%); padding: 20px; border-radius: 10px; margin-bottom: 20px;">
    <h1 style="color: white; margin: 0;">Dashboard Executivo</h1>
    <p style="color: rgba(255,255,255,0.8); margin: 5px 0 0 0;">Progresso e Linha do Tempo por Data-Lake</p>
</div>
""", unsafe_allow_html=True)

_hoje = pd.Timestamp(datetime.now(timezone(timedelta(hours=-3))).date())

# Data de atualização da fonte (máximo de Data Atualizacao no CSV)
_data_atu_fonte = pd.to_datetime(df['Data Atualizacao'], errors='coerce', utc=True).max()
if pd.notna(_data_atu_fonte):
    _data_atu_fonte_brt = _data_atu_fonte.tz_convert('America/Sao_Paulo')
    _data_atu_str = _data_atu_fonte_brt.strftime('%d/%m/%Y %H:%M')
else:
    _data_atu_str = "—"
st.markdown(f"**📅 Dados atualizados em:** {_data_atu_str} (horário de Brasília) | **Fase:** FASE 3")
